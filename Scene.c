...
bool Scene_Update(Scene *self)
{
	...
	if (!self->ui_mode) {
		...
		while (i < self->portalsCount)
		{
			...
			Enemy_Update(enemy);//TODO

			if (enemy->state == ENEMY_DEAD)//TODO
			{
				// Supprime l'ennemi
				Scene_RemoveEnemy(self, i);
				removed = true;
			...
			//Look for a colision.
			if (Vec2_Distance(player->position, portal->position) <= (portal->radius + player->radius)) {
				//Trigger the collision.
				Player_Portal_Collision(player, portal);//TODO
			}

			// Passe au prochain portail
			if (removed == false)
			{
				i++;
			}
